...
(function($, window, document, undefined) {
	...
	var $window = $(window), pluginName = 'waterfall', defaults = {
		...
		bufferPixel : -50, // decrease this number if you want scroll to fire
							// quicker
		...
		callbacks : {
			/*
			 * loading start @param {Object} loading $('#waterfall-loading')
			 */
			...
			/*
			 * loading finished @param {Object} loading $('#waterfall-loading')
			...
			/*
			 * loading error @param {String} xhr , "end" "error"
			 */
			...
			/*
			 * render data @param {String} data @param {String} dataType ,
			 * "json", "jsonp", "html"
			 */
			renderData : function(data, dataType) {
				var tpl, template;

				if (dataType === 'json' || dataType === 'jsonp') { // json or
																	// jsonp
																	// format
	...
	Waterfall.prototype = {
		...
		/*
		 * _init @callback {Object Function } and when instance is triggered
		 * again -> $element.waterfall()
		 */
		...
		_initContainer : function() {
			...
			this.$element.css(this.options.containerStyle).addClass(
					prefix + '-container');
			this.$element.after('<div id="' + prefix + '-loading">'
					+ options.loadingMsg + '</div><div id="' + prefix
					+ '-message" style="text-align:center;color:#999;"></div>');
		...
		_getColumns : function() {
			var options = this.options, $container = options.fitWidth ? this.$element
					.parent()
					: this.$element, containerWidth = $container[0].tagName === 'BODY' ? $container
					.width() - 20
					: $container.width(), // if $container[0].tagName ===
											// 'BODY', fix browser scrollbar
			colWidth = options.colWidth, gutterWidth = options.gutterWidth, minCol = options.minCol, maxCol = options.maxCol, cols = Math
					.floor(containerWidth / (colWidth + gutterWidth)), col = Math
					.max(cols, minCol);

			/*
			 * if ( !maxCol ) { return col; } else { return col > maxCol ?
			 * maxCol : col; }
			 */
		...
		layout : function($content, callback) {
			var options = this.options, $items = this.options.isFadeIn ? this
					._getItems($content).css({
						opacity : 0
					}).animate({
						opacity : 1
					}) : this._getItems($content), styleFn = (this.options.isAnimated && this.options.state.isResizing) ? 'animate'
					: 'css', animationOptions = options.animationOptions, colWidth = options.colWidth, gutterWidth = options.gutterWidth, len = this.colHeightArray.length, align = options.align, fixMarginLeft, obj, i, j, itemsLen, styleLen;
			...
			// fixMarginLeft
			if (align === 'center') {
				fixMarginLeft = (this.$element.width() - colWidth * len - gutterWidth
						* (len - 1)) / 2;
				fixMarginLeft = fixMarginLeft > 0 ? fixMarginLeft : 0;
			} else if (align === 'left') {
				fixMarginLeft = 0;
			} else if (align === 'right') {
				fixMarginLeft = this.$element.width() - colWidth * len
						- gutterWidth * (len - 1);
			}
		...
		_placeItems : function(item, fixMarginLeft) {
			...
			var $item = $(item), options = this.options, colWidth = options.colWidth, gutterWidth = options.gutterWidth, gutterHeight = options.gutterHeight, colHeightArray = this.colHeightArray, len = colHeightArray.length, minColHeight = Math.min
					.apply({}, colHeightArray), minColIndex = $.inArray(
					minColHeight, colHeightArray), colIndex, // cur column
																// index
			position;
		...
		/*
		 * prepend @param {Object} $content @param {Function} callback
		 */
		...
		/*
		 * append @param {Object} $content @param {Function} callback
		 */
		...
		/*
		 * remove item @param {Object} $items @param {Function} callback
		 */
		...
		/*
		 * opts @param {Object} opts @param {Function} callback
		 */
		...
		_requestData : function(callback) {
			var self = this, options = this.options, maxPage = options.maxPage, curPage = options.state.curPage++, // cur
																													// page
			...
			if (maxPage !== undefined && curPage > maxPage) {
				...
				options.callbacks.loadingFinished(this.$loading,
						options.state.isBeyondMaxPage);
				return;
			}
			...
			$.ajax({
				...
				success : function(data) {
					if(data.code=="201") {
						this.$loading = $('#' + options.prefix + '-loading');
						options.state.isBeyondMaxPage = true;
						options.callbacks.loadingFinished(this.$loading,
								options.state.isBeyondMaxPage);
						if(curPage==1)
							this.$message = $('#' + options.prefix + '-message').html("没有找到符合该关键词的图片");
						else
							this.$message = $('#' + options.prefix + '-message').html("已经没有符合该关键词的图片了");
		...
		_handleResponse : function(data, callback) {
			var self = this, options = this.options, content = $
					.trim(options.callbacks.renderData(data, options.dataType)), $content = $(content), checkImagesLoaded = options.checkImagesLoaded;

			if (!checkImagesLoaded) {
				self.append($content, callback);
				self.options.callbacks.loadingFinished(self.$loading,
						self.options.state.isBeyondMaxPage);
			} else {
		...
		_nearbottom : function() {
			var options = this.options, minColHeight = Math.min.apply({},
					this.colHeightArray), distanceFromWindowBottomToMinColBottom = $window
					.scrollTop()
					+ $window.height()
		...
		_scroll : function() {
			...
			if (state.isProcessingData || state.isDuringAjax
					|| state.isInvalidPage || state.isPause) {
				return;
			}
		...
		_doScroll : function() {
			...
			$("#piccontainer_parent").bind('scroll', function() {
				if (scrollTimer) {
					clearTimeout(scrollTimer);
				}
		 ...
		 */
		_resize : function() {
			var cols = this.cols, newCols = this._getColumns(); // new columns
		...
		if (typeof options === 'string') { // plugin method
			...
			this.each(function() {
				if (!$.data(this, 'plugin_' + pluginName)) {
					$.data(this, 'plugin_' + pluginName, new Waterfall(this,
							options));
				}
			});
		}
...
(function($, undefined) {
	...
	$.fn.imagesLoaded = function(callback) {
		var $this = this, deferred = $.isFunction($.Deferred) ? $.Deferred()
				: 0, hasNotify = $.isFunction(deferred.notify), $images = $this
				.find('img').add($this.filter('img')), loaded = [], proper = [], broken = [];
		...
		function imgLoaded(img, isBroken) {
			...
			if (hasNotify) {
				deferred.notifyWith($(img), [ isBroken, $images, $(proper),
						$(broken) ]);
			}
		...
		if (!$images.length) {
			...
			$images.bind('load.imagesLoaded error.imagesLoaded',
					imgLoadedHandler).each(
					function(i, el) {
						var src = el.src,

						// find out if this image has been already checked for
						// status
						// if it was, and src has not changed, call imgLoaded on
						// it
						...
						// if complete is true and browser supports natural
						// sizes, try
						// to check for image status manually
						if (el.complete && el.naturalWidth !== undefined) {
							imgLoaded(el, el.naturalWidth === 0
									|| el.naturalHeight === 0);
							return;
						}

						// cached images don't fire load sometimes, so we reset
						// src, but only when
						// dealing with IE, or image is complete (loaded) and
						// failed manual check
						// webkit hack from
