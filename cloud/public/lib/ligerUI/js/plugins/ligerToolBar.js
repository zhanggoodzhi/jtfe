...
(function ($)
{
    ...
    $.ligerui.controls.ToolBar.ligerExtend($.ligerui.core.UIComponent, {
        ...
        {
        	var g = this, p = this.options;
            if(item.render && item.type == "input"){
        		...
        		var element = $("<" + item.type + " placeholder=" + item.text + " />");
                item.text && item.type && $("span:first", ditem).html(element);
                
                $("span:first", ditem).append("<label for='classify' id='classifyLabel' style='float:left;margin-right:4px;'>" + "关键词" + "</label>");
                ...
                item.focus && element.focus(function () { item.focus(item); });
                item.blur && element.blur(function () { item.blur(item); });
                item.style && element.css(item.style);
                item.bind && element.bind(item.bind.act, function(event){item.bind.fun(event);});
        		...
        		if (item.menu){
                    ...
                    ditem.hover(item.hover.over(item), item.hover.out(item));
                }

            } else if (item.render && item.type == "comboBox"){
        		...
        		var defaultId = "classify";
        		if(item.id){
        			defaultId = item.id;
        		}
        		
        		var element = $("<div class='comboBoxDiv' style='float:left; margin-top: 0px;'><input type='input' id='" + defaultId + "' readonly='' class='l-tex'></div>");
                item.text && item.type && $("span:first", ditem).append("<label for='" + defaultId + "' id='" + defaultId + "Label' style='float:left;margin-right:4px;'>" + item.text + "</label>");
                 $("span:first", ditem).append(element);
                element.find("#" + defaultId).ligerComboBox(item.config);
                item.disable && ditem.addClass("l-toolbar-item-disable");
                item.click && ditem.click(function () { if ($(this).hasClass("l-toolbar-item-disable")) return;item.click(item); });
                item.focus && element.focus(function () { item.focus(item); });
                item.blur && element.blur(function () { item.blur(item); });
