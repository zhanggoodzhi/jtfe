...
var Wweixinv2MenuIndex;
(function (Wweixinv2MenuIndex) {
    var urlReg = new RegExp('^(https?|ftp):\/\/.+\..+$', 'i');
    var viewState;
    (function (viewState) {
        viewState[viewState["common"] = 0] = "common";
        viewState[viewState["hasSub"] = 1] = "hasSub";
        viewState[viewState["noMenu"] = 2] = "noMenu";
    })(viewState || (viewState = {}));
    $(function () {
        initMenus();
        initSave();
    ...
    // 初始化菜单
    function initMenus() {
        var root = $('#weixin-menu-list');
        // 被点击的菜单添加选中样式
        root.on('click', '.weixin-menu-item', function (e) {
            var el = $(e.currentTarget);
            $('.weixin-menu-item.selected').removeClass('selected');
            el.addClass('selected');
        });
        root.on('click', '.weixin-parent-menu', function (e) {
            var el = $(e.currentTarget), subMenu = el.next('.sub-menu-list'), data = el.data(), subNum = getSubMEnuNumber(subMenu);
            fillData(data, false);
        ...
        root.on('click', '.weixin-sub-menu', function (e) {
            var el = $(e.currentTarget), data = el.data();
            viewShow(viewState.common);
            fillData(data, false);
        });
        root.on('click', '.add-parent-menu', function (e) {
            var num = getParentMenuNumber(), el = $(e.currentTarget).parent();
            if (num >= 3 || num < 0) {
                return;
            }
            var newEl = createParentMenuElement();
            setTimeout(function () {
                el.before(newEl);
        ...
        utils.bindInput($('#menu-name'), 'input change', function (val) {
            var el = root.find('.weixin-menu-item.selected');
            el.data('name', val);
            fillData(el.data(), true);
        }, 100);
        utils.bindInput($('#menu-content'), 'input change', function (val) {
            var el = root.find('.weixin-menu-item.selected'), data = el.data();
            if (!data.type) {
                return;
            }
        ...
        $('input:radio[name=type]').on('change', function (e) {
            var el = root.find('.weixin-menu-item.selected'), data = el.data(), target = $(e.currentTarget), val = target.val();
        ...
        root.on('click', '.add-sub-menu', function (e) {
            var el = $(e.currentTarget).parent(), num = getSubMEnuNumber(el.parent('.sub-menu-list'));
            if (num >= 5 || num < 0) {
                return;
            }
            var newEl = createSubMenuElement();
            setTimeout(function () {
                el.before(newEl);
                if (num === 4) {
                    el.remove();
            ...
            utils.confirmModal({
                ...
                cb: function (modal) {
                    if (el.hasClass('weixin-parent-menu')) {
                        var num = getParentMenuNumber();
                        if (num === 3) {
                            $('#weixin-menu-list').append(createParentMenuElement(true));
                        }
                        el.parent().remove();
                        var currentParentMenus = $('#weixin-menu-list .weixin-parent-menu:not(.add-parent-menu)');
                        ...
                        var subList = el.parents('.sub-menu-list');
                        var num = getSubMEnuNumber(subList);
                        if (num === 5) {
                            subList.append(createSubMenuElement(true));
                        }
                        el.parent().remove();
                        var currentSubMenus = subList.find('.weixin-sub-menu:not(.add-sub-menu)');
                        if (currentSubMenus.length > 0) {
                            currentSubMenus.first().click();
                        }
                        else {
                            var p = subList.prev('.weixin-parent-menu');
                            if (!p.data('type')) {
                                p.data('type', 'view');
                            }
                            p.trigger('click');
        ...
        $('#weixin-resync').on('click', function () {
            utils.confirmModal({
        ...
        root.find('.sub-menu-list').toArray().forEach(function (v) {
            new Sortable(v, {
                sort: true
            });
        });
    ...
    function fillData(data, flag) {
        ...
        if (data.type) {
            var el = $("input:radio[name=type][value=" + data.type + "]");
    ...
    function createSubMenuElement(add) {
        if (add === void 0) { add = false; }
        var el;
        if (!add) {
    ...
    function initSave() {
        ...
        btn.on('click', function () {
            var parentElement = $('#weixin-menu-list .weixin-parent-menu:not(.add-parent-menu)').toArray(), d = [];
            for (var _i = 0, parentElement_1 = parentElement; _i < parentElement_1.length; _i++) {
                var i = parentElement_1[_i];
                var iEl = $(i), subElement = iEl.next('.sub-menu-list').find('.weixin-sub-menu:not(.add-sub-menu)').toArray();
                var parentElementData = void 0;
                ...
                if (subElement.length > 0) {
                    for (var _a = 0, subElement_1 = subElement; _a < subElement_1.length; _a++) {
                        var j = subElement_1[_a];
                        var jEl = $(j), subElementData = renderSaveData(jEl.data(), parentElementData.priority + subElement.indexOf(j) + 1);
                        if (!subElementData) {
                            return;
                        }
            ...
            d.sort(function (x, y) { return x.priority - y.priority; });
            var data = {
                credentialId: credentialId,
                data: d
            };
    ...
    function renderSaveData(data, id, ignore) {
    ...
    function viewShow(state) {
        var detial = $('#menu-detial'), info = $('#menu-info'), infoText = $('#menu-info-text'), formEl = $('#weixin-edit-container input,textarea').prop('disabled', null);
        switch (state) {
...
})(Wweixinv2MenuIndex || (Wweixinv2MenuIndex = {}));
