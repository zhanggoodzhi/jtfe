...
(function (Appswitch) {
    ...
    function initTable() {
        $('#table').DataTable(Object.assign(tables.commonConfig('single'), {
            ajax: {
                type: 'POST',
                url: 'applist',
    ...
    function initComplete() {
        ...
        table.on('select', function (e, dt) {
            var data = tables.getSelected(dt)[0];
            if (!data) {
                return;
            }
            var endLoading = utils.addLoadingBg($('.x_panel'), $('.x_content'));
            $.ajax({
                type: 'POST',
                url: 'chooseApp',
                data: {
        ...
        utils.bindInput($('#app-number'), 'input', function (val) {
            $('#app-number').val(val.replace(/\D/g, ''));
        }, 100);
        utils.bindEnter($('#app-number,#app-name'), function () {
            t.refresh(true);
            // table.draw();
        });
    }
    function renderStatus(status) {
        for (var _i = 0, appStatus_1 = appStatus; _i < appStatus_1.length; _i++) {
            var v = appStatus_1[_i];
            if (status === v.code) {
                return v.name;
            }
        }
