		...
		options: {
			...
			initComplete: () => {
				...
				$('#answer-del-btn').on('click', () => {
					...
					if (data.length <= 0) {
						utils.alertMessage('请选择要删除的重复问题回复');
						return;
					}
				...
				$('#answer-edit-btn').on('click', () => {
					...
					if (data.length < 1) {
						utils.alertMessage('请选择要编辑的重复问题回复');
					}
					else if (data.length > 1) {
						utils.alertMessage('只能编辑一个重复问题回复');
					}
					else {
...
export function submitDefaultAnswer(data, edit?: boolean) {
	const url = edit ? 'setting/app/repeatQ/update' : 'setting/app/repeatQ/add';
