...
export function initMusic(p: resourceUtils.PushModal) {
...
function init() {
	...
	table = new tables.Table({
		...
		options: {
			...
			// 			data:$('#music-search-title').val().trim()
			// 		});
			...
			columns: [
				...
				{ data: 'createTime', title: '创建时间', createdCell: tables.createAddTitle, render: utils.renderCommonTime },
				{
...
// 	return `
// 		<i title="编辑" data-id='${data}' class='fa cloud-fa-icon fa-edit view-edit'></i>
// 		<i title="推送到矩阵" data-id='${data}' class='fa cloud-fa-icon fa-sign-out view-edit'></i>
// 		<i title="群发" data-id='${data}' class='fa cloud-fa-icon fa-share-square-o view-edit'></i>
// 		<i title="删除" data-id='${data}' class='fa cloud-fa-icon fa-trash view-delete'></i>
// 	`;
// }
...
function bindAddMusic(pEl) {
	pEl.on('click', '#music-add-btn', (event) => {
		state = 'create';
		detailEl.find('.sidebar-title').text('添加音乐素材');
		initUpload();
		detail.show();
		event.stopPropagation();
	});
...
function hideDel() {
	if (saveFlag) {
		resourceUtils.resourceFileDeleteMoreEvent(toDeleteArr);
	} else {
		resourceUtils.resourceFileDeleteMoreEvent(uploadedArr);
	}
	clearMusicForm();
	saveFlag = false;
}
